...
ms.locfileid: "73021874"
---
# <a name="populating-a-table-with-data-in-xamarinios"></a>Rellenar una tabla con datos en Xamarin. iOS

Para agregar filas a un `UITableView` debe implementar una subclase `UITableViewSource` e invalidar los métodos a los que llama la vista de tabla para rellenarse.
...
Un `UITableViewSource` puede utilizar cualquier estructura de datos, desde una matriz de cadenas simple (como se muestra en este ejemplo) hasta una lista < > u otra colección. La implementación de métodos `UITableViewSource` aísla la tabla de la estructura de datos subyacente.

Para usar esta subclase, cree una matriz de cadenas para construir el origen y luego asígnela a una instancia de `UITableView`:
...
Por último, de nuevo en el método de `RowSelected`, llame a `PresentViewController` en el campo almacenado en caché:
...
En este ejemplo, solo hay seis elementos, por lo que no se requiere ninguna reutilización de celdas. Sin embargo, cuando se muestran cientos o miles de filas, sería una pérdida de memoria para crear cientos o miles de objetos de `UITableViewCell` cuando solo se ajustan unos pocos en la pantalla a la vez.

Para evitar esta situación, cuando una celda desaparece de la pantalla, su vista se coloca en una cola para su reutilización. A medida que el usuario se desplaza, la tabla llama `GetCell` para solicitar nuevas vistas para mostrar: para reutilizar una celda existente (que no se está mostrando actualmente), simplemente llame al método `DequeueReusableCell`. Si una celda está disponible para su reutilización, se devolverá; de lo contrario, se devolverá un valor NULL y el código deberá crear una nueva instancia de celda.
...
Con el nuevo patrón, una aplicación registra la clase de celda o Xib que se va a usar llamando a `RegisterClassForCellReuse` o `RegisterNibForCellReuse` en el constructor del controlador. A continuación, al quitar de la cola la celda en el método `GetCell`, simplemente llame a `DequeueReusableCell` pasando el identificador registrado para la clase de celda o Xib y la ruta de acceso del índice.
...
Una vez registrada la clase celda, la celda se puede quitar de la cola en el método `GetCell` del `UITableViewSource` sin necesidad de la comprobación nula adicional, como se muestra a continuación:
...
Para admitir ' secciones ', los datos subyacentes de la tabla deben agruparse, por lo que el ejemplo BasicTableIndex crea una `Dictionary<>` a partir de la matriz de cadenas usando la primera letra de cada elemento como clave del diccionario:
...
A continuación, la subclase `UITableViewSource` necesita que se agreguen o modifiquen los métodos siguientes para usar el `Dictionary<>`:
...
Puede personalizar aún más la apariencia del encabezado y del pie de página con un objeto de vista, mediante las invalidaciones del método `GetViewForHeader` y `GetViewForFooter` en `UITableViewSource`.
