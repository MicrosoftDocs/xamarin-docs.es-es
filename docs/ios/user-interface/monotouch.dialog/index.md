...
*DialogViewController*, o *DVC* for short, hereda de `UITableViewController` y, por tanto, representa una pantalla con una tabla. DVCs se puede insertar en un controlador de navegación como un UITableViewController normal.
...
Si un RootElement se inicializa con un valor de sección o elemento, este valor se usa para buscar un elemento secundario que proporcionará un resumen de la configuración, que se representa en el lado derecho de la pantalla. Por ejemplo, en la siguiente captura de pantalla se muestra una tabla a la izquierda con una celda que contiene el título de la pantalla de detalles a la derecha, "postre", junto con el valor del desierto seleccionado.
 ...
 [![](images/image4.png "En esta captura de pantalla se muestra una tabla a la izquierda con una celda que contiene el título de la pantalla de detalles de la derecha, el postre, junto con el valor del desierto seleccionado.")](images/image4.png#lightbox)[![](images/image5.png "En esta captura de pantalla siguiente se muestra una tabla a la izquierda con una celda que contiene el título de la pantalla de detalles de la derecha, el postre, junto con el valor del desierto seleccionado.")](images/image5.png#lightbox)
...
Módulo. D muestra una `NSAction` como un delegado para controlar las devoluciones de llamada.
...
Combinado con la propiedad `Element.Value`, la devolución de llamada puede recuperar el valor establecido en otros elementos. Por ejemplo, considere el siguiente código:
...
Cuando el usuario presiona la celda de la tabla inferior, se ejecuta el código de la función anónima, escribiendo el valor de la instancia de `element` en el panel de salida de la **aplicación** en Visual Studio para Mac.
...
Los elementos que se usan para capturar datos proporcionados por el usuario exponen una propiedad de `Value` pública que contiene el valor actual del elemento en cualquier momento. Se actualiza automáticamente a medida que el usuario usa la aplicación.
...
La clase `StyledStringElement` se deriva de `StringElement`, pero permite a los desarrolladores personalizar unas cuantas propiedades como la fuente, el color del texto, el color de la celda de fondo, el modo de salto de línea, el número de líneas que se van a mostrar y si se debe mostrar un accesorio.
...
El `EntryElement`, como el nombre implica, se usa para obtener los datos proporcionados por el usuario. Admite cadenas normales o contraseñas, donde los caracteres están ocultos.
...
En cualquier momento, el acceso a su propiedad Value puede recuperar el valor de la `EntryElement`.

Además, la propiedad `KeyboardType` se puede establecer en el momento de la creación en el estilo de tipo de teclado deseado para la entrada de datos. Se puede usar para configurar el teclado mediante los valores de `UIKeyboardType` como se indica a continuación:
...
Un `RadioElement` requiere que se especifique un `RadioGroup` en la `RootElement`.
...
En el `HTMLElement` se muestra el valor de su propiedad `Caption` en la celda de la tabla. Al seleccionado, el `Url` asignado al elemento se carga en un control `UIWebView`, como se muestra a continuación:
...
El indicador de `UIActivity` comienza a animar y el título de carga se muestra cuando un usuario pulsa la celda y, a continuación, se ejecuta el `NSAction` que se pasa al constructor. Una vez finalizado el código del `NSAction`, el indicador de `UIActivity` detiene la animación y se vuelve a mostrar el título normal.

### <a name="uiview-element"></a>Elemento UIView

Además, cualquier `UIView` personalizado se puede mostrar mediante el `UIViewElement`.

### <a name="owner-drawn-element"></a>Elemento dibujado por el propietario

Este elemento debe ser de subclase, ya que es una clase abstracta. Debe reemplazar el método de `Height(RectangleF bounds)` en el que debe devolver el alto del elemento, así como `Draw(RectangleF bounds, CGContext context, UIView view)` en el que debe hacer todo el dibujo personalizado dentro de los límites especificados, mediante los parámetros de contexto y vista. Este elemento realiza el pesado trabajo de subclases de una `UIView`y la coloca en la celda que se va a devolver, lo que solo necesita implementar dos invalidaciones simples. Puede ver una mejor implementación de ejemplo en la aplicación de ejemplo en el archivo de `DemoOwnerDrawnElement.cs`.
...
El `JsonElement` es una subclase de `RootElement` que extiende un `RootElement` para poder cargar el contenido de un elemento secundario anidado desde una dirección URL local o remota.

El `JsonElement` es un `RootElement` en el que se pueden crear instancias de dos formas. Una versión crea un `RootElement` que cargará el contenido a petición. Estos se crean mediante el `JsonElement` constructores que toman un argumento adicional al final, la dirección URL de la que se va a cargar el contenido:

```csharp
var je = new JsonElement ("Dynamic Data", "https://tirania.org/tmp/demo.json");
```

El otro formulario crea los datos a partir de un archivo local o de un `System.Json.JsonObject` existente que ya ha analizado:
...
Para agregar compatibilidad automática de extracción a actualización a los cuadros de diálogo, solo tiene que hacer dos cosas: enlazar un controlador de eventos para recibir una notificación cuando el usuario extrae los datos y notificar al `DialogViewController` cuando se han cargado los datos para volver a su estado predeterminado.

Enlazar una notificación es sencillo. solo tiene que conectarse al evento de `RefreshRequested` en el `DialogViewController`, de la siguiente manera:

```csharp
dvc.RefreshRequested += OnUserRequestedRefresh;
```

Después, en el método `OnUserRequestedRefresh`, se pone en cola parte de la carga de datos, se solicitan algunos datos de la red o se gira un subproceso para calcular los datos. Una vez que se han cargado los datos, debe notificar a la `DialogViewController` en la que se encuentra el nuevo dato y restaurar la vista a su estado predeterminado; para ello, llame a `ReloadComplete`:
...
Para admitir la búsqueda, establezca la propiedad `EnableSearch` en el `DialogViewController`. También puede establecer la propiedad `SearchPlaceholder` que se va a usar como texto de marca de agua en la barra de búsqueda.

La búsqueda cambiará el contenido de la vista a medida que el usuario escribe. Busca en los campos visibles y los muestra al usuario. El `DialogViewController` expone tres métodos para iniciar, terminar o desencadenar una nueva operación de filtro en los resultados mediante programación. Estos métodos se enumeran a continuación:
...
El cargador de imágenes se implementa en la clase `ImageLoader`, lo único que debe hacer es llamar al método `DefaultRequestImage`, deberá proporcionar el URI de la imagen que desea cargar, así como una instancia de la interfaz `IImageUpdated` que se invocará cuando la imagen se ha cargado.
...
Si planea implementar el método de `GetCell` llamando a `base.GetCell(tv)` y personalizar la celda devuelta, también debe invalidar la propiedad `CellKey` para devolver una clave que será única para el elemento, como en este ejemplo:
...
Esto funciona en la mayoría de los elementos, pero no en el `StringElement` y `StyledStringElement`, ya que utilizan su propio conjunto de claves para varios escenarios de representación. Tendría que replicar el código en esas clases.

### <a name="dialogviewcontrollers-dvcs"></a>DialogViewControllers (DVCs)

Tanto la API de reflexión como la de elementos usan el mismo `DialogViewController`. A veces, querrá personalizar el aspecto de la vista o puede que desee usar algunas características de la `UITableViewController` que van más allá de la creación básica de interfaces de usuario.

La `DialogViewController` es simplemente una subclase de la `UITableViewController` y puede personalizarla de la misma manera que personalizaría una `UITableViewController`.
...
En el caso de personalizaciones más avanzadas de la `DialogViewController`, como establecer su fondo, la subclase y la invalidación de los métodos adecuados, tal como se muestra en el ejemplo siguiente:
...
Otro punto de personalización son los siguientes métodos virtuales en el `DialogViewController`:
...
Puede usar esta invalidación para capturar cualquiera de los métodos de `UITableViewSource`. Por ejemplo, [TweetStation](https://github.com/migueldeicaza/TweetStation) lo utiliza para realizar un seguimiento cuando el usuario se ha desplazado a la parte superior y se actualiza en consecuencia el número de tweets no leídos.
...
Si desea realizar la validación de datos, debe hacerlo cuando el usuario desencadene una acción con los datos especificados. Por ejemplo, un botón **listo** o **siguiente** en la barra de herramientas superior, o algunos `StringElement` usar como botón para ir a la siguiente fase.
...
La forma de notificar un error al usuario es específica de la aplicación. Podría aparecer un `UIAlertView` o mostrar una sugerencia.
