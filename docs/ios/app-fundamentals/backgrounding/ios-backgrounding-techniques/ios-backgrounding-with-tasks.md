...
1. **Tareas de DidEnterBackground** : se llama durante el método de ciclo de vida de la aplicación `DidEnterBackground` para ayudar en la limpieza y el guardado del estado.
...
Las tareas de `DidEnterBackground` y con seguridad en segundo plano se pueden usar en iOS 6 e iOS 7, con algunas pequeñas diferencias. Vamos a investigar estos dos tipos de tareas con mayor detalle.
...
El proceso de registro empareja una tarea con un identificador único, `taskID`y, a continuación, la ajusta en `BeginBackgroundTask` y llamadas `EndBackgroundTask` coincidentes. Para generar el identificador, se realiza una llamada al método `BeginBackgroundTask` en el objeto `UIApplication` y, a continuación, se inicia la tarea de ejecución prolongada, normalmente en un nuevo subproceso. Una vez completada la tarea, llamamos a `EndBackgroundTask` y pasamos el mismo identificador. Esto es importante porque iOS finalizará la aplicación si una llamada `BeginBackgroundTask` no tiene una `EndBackgroundTask`coincidente.
...
Además de proporcionar acceso a la propiedad `BackgroundTimeRemaining`, iOS proporciona una manera adecuada de controlar la expiración del tiempo de fondo a través de un **controlador de expiración**. Este es un bloque opcional de código que se ejecutará cuando el tiempo asignado a una tarea esté a punto de expirar. El código del controlador de expiración llama a `EndBackgroundTask` y pasa el identificador de tarea, lo que indica que la aplicación se comporta bien y evita que iOS finalice la aplicación incluso si la tarea se ejecuta sin tiempo. se debe llamar a `EndBackgroundTask` dentro del controlador de expiración, así como en el curso normal de la ejecución. 
...
Dado que el tiempo de ejecución de la tarea ya no es continuo, las tareas que realizan transferencias de red deben administrarse de manera diferente en iOS 7. Se recomienda a los desarrolladores que usen el `NSURlSession` API para controlar las transferencias de red. La siguiente sección es una introducción a las transferencias en segundo plano.

 <a name="background-transfers" />

## <a name="background-transfers"></a>Transferencias en segundo plano

La red troncal de las transferencias en segundo plano en iOS 7 es la nueva API de `NSURLSession`. `NSURLSession` nos permite crear tareas para:
...
La API de `NSURLSession` crea una o varias sesiones que, a su vez, generan tareas para los bloques de datos relacionados a través de la red. Las tareas se ejecutan de forma asincrónica para transferir datos de forma rápida y confiable. Dado que `NSURLSession` es asincrónico, cada sesión requiere un bloque de controlador de finalización para permitir que el sistema y la aplicación sepan cuándo se completa una transferencia.

Para realizar una transferencia de red que sea válida en las versiones anteriores a iOS 7 y posteriores a iOS 7, compruebe si hay un `NSURLSession` disponible para poner en cola las transferencias y use una tarea en segundo plano normal para realizar la transferencia si no lo está:
