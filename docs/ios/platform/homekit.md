...
[![](homekit-images/accessory01.png "Una aplicación habilitada para HomeKit de ejemplo")](homekit-images/accessory01.png#lightbox)
...
- `HMRoom`-Aunque es opcional, `HMRoom` permite al usuario definir salones específicos dentro de un inicio (`HMHome`) como: Cocina, baño, garaje o salón. El usuario puede agrupar todos los dispositivos de automatización doméstica en una ubicación específica de su casa en `HMRoom` un y actuar sobre ellos como una unidad. Por ejemplo, pedir a Siri que desactive las luces del garaje.
...
6. Haga clic en **aprovisionar perfiles** > **desarrollo** y cree un nuevo perfil de aprovisionamiento de desarrollo para la aplicación: 
    ...
    [![](homekit-images/provision02.png "Crear un nuevo perfil de aprovisionamiento de desarrollo para la aplicación")](homekit-images/provision02.png#lightbox)
...
2. Descargue las **herramientas adicionales para Xcode XXX** (donde xxx es la versión de Xcode que ha instalado): 
    ...
    [![](homekit-images/simulator01.png "Descarga de las herramientas adicionales para Xcode")](homekit-images/simulator01.png#lightbox)
...
1. En la carpeta Aplicaciones, inicie el simulador accesorio HomeKit: 
    ...
    [![](homekit-images/simulator02.png "El simulador accesorio HomeKit")](homekit-images/simulator02.png#lightbox)
...
4. Haga clic en **Agregar servicio.** y seleccione un tipo de servicio en la lista desplegable: 
    ...
    [![](homekit-images/simulator05.png "Seleccionar un tipo de servicio en la lista desplegable")](homekit-images/simulator05.png#lightbox)
...
6. Puede proporcionar características opcionales para un servicio haciendo clic en el botón **Agregar característica** y configurando las opciones necesarias: 
    ...
    [![](homekit-images/simulator07.png "Configuración de los valores obligatorios")](homekit-images/simulator07.png#lightbox)
...
4. En la lista desplegable, seleccione **privacidad-HomeKit uso Descripción**: 
    ...
    [![](homekit-images/info02.png "Seleccionar privacidad: Descripción de uso de HomeKit")](homekit-images/info02.png#lightbox)
...
> [!IMPORTANT]
> Si no se establece `NSHomeKitUsageDescription` la clave en `Info.plist` el archivo se producirá un error en la aplicación en _modo silencioso_ (cerrado por el sistema en tiempo de ejecución) sin errores cuando se ejecute en iOS 10 (o superior).
...
[![](homekit-images/home01.png "Se le preguntará al usuario si desea permitirle acceder a su información de HomeKit")](homekit-images/home01.png#lightbox)
...
Cuando la aplicación se inicia por primera vez o vuelve desde el fondo, debe supervisar `DidUpdateHomes` el evento de `HMHomeManager` la clase para comprobar la existencia de una página principal principal. Si no existe ninguna, debe proporcionar una interfaz para que el usuario cree una.
...
Cuando el administrador de inicios realiza una conexión a HomeKit `DidUpdateHomes` , se desencadena el evento, se cargarán todos los hogares existentes en la colección de hogares del administrador y se cargará la Página principal principal, si está disponible.

### <a name="adding-a-primary-home"></a>Agregar una página principal principal

Si la `PrimaryHome` `HMHomeManager` propiedad de se encuentra `null` después de `DidUpdateHomes` un evento, debe proporcionar una manera para que el usuario cree y asigne una página principal principal antes de continuar.

Normalmente, la aplicación presentará un formulario para que el usuario asigne un nombre a una nueva página principal que, a continuación, se pasa al administrador principal para configurar como principal. En el caso de la aplicación de ejemplo **HomeKitIntro** , se creó una vista modal en el diseñador de `AddHomeSegue` iOS y la llamó segue desde la interfaz principal de la aplicación.
...
El `AddHome` método intentará crear un nuevo inicio y devolverlo a la rutina de devolución de llamada especificada. Si la `error` propiedad no `null`es, se ha producido un error y se debe presentar al usuario. Los errores más comunes se deben a que el nombre de casa no es único o que el administrador de inicio no puede comunicarse con HomeKit.

Si la Página principal se creó correctamente, debe llamar al `UpdatePrimaryHome` método para establecer el nuevo inicio como principal. De nuevo, si `error` la propiedad no `null`es, se ha producido un error y se debe presentar al usuario.

También debe supervisar los eventos y `DidAddHome` `DidRemoveHome` del administrador principal y actualizar la interfaz de usuario de la aplicación según sea necesario.
...
Una vez que se ha definido o cargado una página principal principal desde el administrador de inicio, la aplicación de Xamarin `HMAccessoryBrowser` . iOS puede llamar a para buscar cualquier nuevo accesorio de automatización de inicio y agregarlo a un hogar.

Llame al `StopSearchingForNewAccessories` método para empezar a buscar nuevos accesorios y el método cuando termine. `StartSearchingForNewAccessories`
...
Se llamará al `DiscoveredAccessories` eventocuandosedetectennuevosaccesoriosyseagregaránalalistaenelexploradordeaccesorios.`DidFindNewAccessory`
...
[![](homekit-images/accessory01.png "Búsqueda de un nuevo accesorio")](homekit-images/accessory01.png#lightbox)
...
Si la `err` propiedad no `null`es, se ha producido un error y se debe presentar al usuario. De lo contrario, se le pedirá al usuario que escriba el código de instalación del dispositivo que se va a agregar:

[![](homekit-images/accessory02.png "Escriba el código de configuración para el dispositivo que se va a agregar")](homekit-images/accessory02.png#lightbox)

En el simulador accesorio HomeKit, este número se puede encontrar en el campo Código de la **instalación** :

[![](homekit-images/accessory03.png "El campo de código de instalación del simulador accesorio HomeKit")](homekit-images/accessory03.png#lightbox)
...
Debe supervisar el evento del `DidRemoveNewAccessory` explorador de accesorios y actualizar la interfaz de usuario para quitar un accesorio de la lista disponible una vez que el usuario lo haya agregado a su colección principal.
...
Los siguientes tipos de servicio están disponibles a `HMServiceType` través de la enumeración:
...
Para asegurarse de que la aplicación tiene el estado más reciente de una característica determinada, `ReadValue` llame al método `HMCharacteristic` de la clase. Si la `err` propiedad no `null`es, se ha producido un error y puede o no presentarse al usuario.

La propiedad de `Value` la característica contiene el estado actual de la característica determinada `NSObject`como y, como tal, no se puede trabajar directamente C#con en.
...
Para modificar un `HMCharacteristic`, llame a `WriteValue` su método y ajuste el nuevo valor en `NSObject.FromObject` una llamada. Por ejemplo:
...
Si la `err` propiedad no `null`es, se ha producido un error y se debe presentar al usuario.

### <a name="testing-characteristic-value-changes"></a>Probar cambios de valores característicos

Al trabajar con `HMCharacteristics` los accesorios de y simulados, las `Value` modificaciones en la propiedad se pueden supervisar dentro del simulador de accesorios de HomeKit.
...
[![](homekit-images/test01.png "Cambiar el estado de una luz en una aplicación iOS")](homekit-images/test01.png#lightbox)
...
- **Identificadores persistentes** : todas las clases de HomeKit pertinentes `UniqueIdentifier` incluyen ahora una propiedad para identificar de forma única un elemento determinado en las aplicaciones habilitadas para HomeKit (o las instancias de la misma aplicación).
...
- Las escenas predefinidas de **escenas** predefinidas se han creado para cuatro eventos comunes que se producen para el usuario de HomeKit medio: Póngase en marcha, déjelo, regrese a la cama. Estas escenas predefinidas no se pueden eliminar de un hogar.
