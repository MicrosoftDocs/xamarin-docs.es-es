...
Para realizar llamadas de subproceso de interfaz de usuario en código compartido, siga el [IDispatchOnUIThread ejemplo](https://www.slideshare.net/follesoe/cross-platform-mobile-apps-using-net) (cortesía de [ @follesoe ](https://twitter.com/follesoe)). Declarar y un programa para un `IDispatchOnUIThread` interfaz en el código compartido y, a continuación, implementar las clases específicas de la plataforma, como se muestra aquí:
...
Deben usar los desarrolladores de Xamarin.Forms [ `Device.BeginInvokeOnMainThread` ](~/xamarin-forms/platform/device.md#interact-with-the-ui-from-background-threads) en el código común (proyectos compartidos o PCL).
