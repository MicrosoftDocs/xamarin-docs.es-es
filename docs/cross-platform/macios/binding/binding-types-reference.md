...
Cada interfaz de la definición de contrato que tiene el [`[BaseType]`](#BaseTypeAttribute) atributo que declara el tipo base para el objeto generado. En la declaración anterior, se generará un tipo de clase C# `MyType` que se enlaza a un tipo de Objective-C denominado`MyType`.

Si especifica tipos después del TypeName (en el ejemplo anterior `Protocol1` y `Protocol2`) mediante la sintaxis de herencia de interfaz, el contenido de esas interfaces se insertará como si hubieran sido parte del contrato para `MyType`.
...
En el ejemplo anterior, puede ver que hemos elegido sobrescribir dos métodos, una notificación de que se ha producido un evento de desplazamiento y la segunda, que es una devolución de llamada que debe devolver un valor booleano que indica al `scrollView` si debe desplazarse a la parte superior o n OTR.
...
Dado que los eventos no devuelven valores (tienen un tipo de valor devuelto nulo) puede conectar varias copias. El `ShouldScrollToTop` no es un evento, sino que es una propiedad con el tipo `UIScrollViewCondition` que tiene esta firma:

```csharp
public delegate bool UIScrollViewCondition (UIScrollView scrollView);
```

Devuelve un valor `bool`, en este caso la sintaxis lambda nos permite devolver simplemente el valor de la función `MakeDecision`.

El generador de enlaces admite la generación de eventos y propiedades que vinculan una clase como `UIScrollView` con su `UIScrollViewDelegate` (bien llamarlos a la clase de modelo), esto se hace anotando la definición de [`[BaseType]`](#BaseTypeAttribute) con los parámetros `Events` y `Delegates` ( se describe a continuación). Además de anotar el [`[BaseType]`](#BaseTypeAttribute) con esos parámetros, es necesario informar al generador de algunos componentes más.

En el caso de los eventos que toman más de un parámetro (en Objective-C, la Convención es que el primer parámetro de una clase de delegado es la instancia del objeto de remitente) debe proporcionar el nombre que desea para la clase `EventArgs` generada. Esto se hace con el atributo [`[EventArgs]`](#EventArgsAttribute) en la declaración del método en la clase de modelo. Por ejemplo:
...
La declaración anterior generará una clase `UIImagePickerImagePickedEventArgs` que se deriva de `EventArgs` y empaqueta ambos parámetros, el `UIImage` y el `NSDictionary`. El generador produce lo siguiente:
...
Además del atributo [`[DefaultValue]`](#DefaultValueAttribute) , también puede usar el atributo [`[DefaultValueFromArgument]`](#DefaultValueFromArgumentAttribute) que dirige el generador para devolver el valor del parámetro especificado en la llamada o el parámetro [`[NoDefaultValue]`](#NoDefaultValueAttribute) que indica al generador que hay no hay ningún valor predeterminado.
...
Use la propiedad `Name` para controlar el nombre con el que se enlazará este tipo en el mundo de Objective-C. Normalmente se usa para dar al C# tipo un nombre que es compatible con las directrices de diseño de .NET Framework, pero que se asigna a un nombre en Objective-C que no sigue esa Convención.

Por ejemplo, en el siguiente caso, asignamos el tipo de `NSURLConnection` de Objective-C a `NSUrlConnection`, ya que las directrices de diseño de .NET Framework usan "URL" en lugar de "URL":
...
El nombre especificado se utiliza como valor para el atributo de `[Register]` generado en el enlace. Si no se especifica `Name`, se usa el nombre corto del tipo como valor del atributo `[Register]` en la salida generada.

#### <a name="basetypeevents-and-basetypedelegates"></a>BaseType. Events y BaseType. Delegates

Estas propiedades se utilizan para controlar la generación de C#eventos de estilo en las clases generadas. Se usan para vincular una clase determinada a su clase de delegado de Objective-C. Encontrará muchos casos en los que una clase usa una clase delegada para enviar notificaciones y eventos. Por ejemplo, un `BarcodeScanner` tendría una `BardodeScannerDelegate` clase complementaria. Normalmente, la clase `BarcodeScanner` tendría una propiedad `Delegate` a la que asignaría una instancia de `BarcodeScannerDelegate`, mientras que esto funciona, es posible que desee exponer a la interfaz C#de eventos de estilo similar a los usuarios y, en esos casos, usaría los`Events`y`Delegates`propiedades del atributo [`[BaseType]`](#BaseTypeAttribute) .

Estas propiedades siempre se establecen juntas y deben tener el mismo número de elementos y mantenerse sincronizados. La matriz de `Delegates` contiene una cadena para cada delegado débilmente tipado que se desea encapsular y la matriz de `Events` contiene un tipo para cada tipo que se desea asociar a él.
...
Si aplica este atributo cuando se crean nuevas instancias de esta clase, la instancia de ese objeto se conservará hasta que se invoque el método al que hace referencia el `KeepRefUntil`. Esto resulta útil para mejorar la facilidad de uso de las API, cuando no se desea que el usuario mantenga una referencia a un objeto para usar el código. El valor de esta propiedad es el nombre de un método en la clase `Delegate`, por lo que debe usarse en combinación con las propiedades `Events` y `Delegates` también.
...
Cuando este atributo se aplica a la definición de interfaz, generará un `[DesignatedInitializer]` atributo en el constructor predeterminado (generado), que se asigna al selector de `init`.
...
Para enlazarlos, puede usar el atributo [`[Category]`](#CategoryAttribute) en una definición de interfaz.   Cuando se usa el atributo [`[Category]`](#CategoryAttribute) , el significado del atributo [`[BaseType]`](#BaseTypeAttribute) cambia de se usa para especificar la clase base que se va a extender, hasta ser el tipo que se va a extender.
...
Lo anterior creará un `MyUIViewExtension` una clase que contiene el método de extensión `MakeBackgroundRed`.   Esto significa que ahora puede llamar a `MakeBackgroundRed` en cualquier subclase de `UIView`, lo que le proporciona la misma funcionalidad que obtendría en Objective-C.
...
Esto es incorrecto porque para usar la extensión de `BoolMethod` necesita una instancia de `FooObject` pero está enlazando una extensión **estática** ObjC, este es un efecto secundario debido al hecho de que se C# implementan los métodos de extensión.
...
La recomendación de evitar esto es alinear la definición de `BoolMethod` dentro de la propia definición de la interfaz de `FooObject`, lo que le permitirá llamar a esta extensión como está pensada `FooObject.BoolMethod (range)`.
...
Se emitirá una advertencia (BI1117) siempre que se encuentre un miembro de [`[Static]`](#StaticAttribute) dentro de una definición de [`[Category]`](#CategoryAttribute) . Si realmente desea tener [`[Static]`](#StaticAttribute) miembros dentro de las definiciones de [`[Category]`](#CategoryAttribute) puede silenciar la advertencia mediante `[Category (allowStaticMembers: true)]` o decorando el miembro o la definición de la interfaz de [`[Category]`](#CategoryAttribute) con [`[Internal]`](#InternalAttribute).

<a name="StaticAttribute_Class" />

### <a name="staticattribute"></a>StaticAttribute

Cuando este atributo se aplica a una clase, simplemente genera una clase estática, una que no se deriva de `NSObject`, por lo que se omite el atributo [`[BaseType]`](#BaseTypeAttribute) . Las clases estáticas se usan para hospedar las variables públicas de C que desea exponer.
...
En general, esto significa que, cuando se subclase de una clase que se ha marcado con el `ModelAttribute`, no se debe llamar al método base.   Llamar a ese método producirá una excepción; se supone que se implementa todo el comportamiento en la subclase para los métodos que reemplace.

<a name="AbstractAttribute" />

### <a name="abstractattribute"></a>AbstractAttribute

De forma predeterminada, los miembros que forman parte de un protocolo no son obligatorios. Esto permite a los usuarios crear una subclase del objeto `Model` simplemente derivando de la clase en C# y reemplazando solo los métodos que le interesan. A veces, el contrato de Objective-C requiere que el usuario proporcione una implementación para este método (que se marcan con la Directiva de `@required` en Objective-C). En esos casos, debe marcar esos métodos con el atributo `[Abstract]`.

El atributo `[Abstract]` se puede aplicar a métodos o propiedades y hace que el generador Marque el miembro generado como abstracto y que la clase sea una clase abstracta.
...
Por ejemplo, en la siguiente clase de delegado imaginaria para una clase `Camera`, se proporciona un `ShouldUploadToServer` que se expondría como una propiedad en la clase `Camera`. Si el usuario de la clase `Camera` no establece explícitamente el valor en una expresión lambda que pueda responder a true o false, el valor predeterminado devuelto en este caso sería false, el valor que se especificó en el atributo `DefaultValue` :
...
En el caso anterior, si el usuario de la clase `NSAnimation` decidió usar cualquiera de los C# eventos o propiedades y no estableció`NSAnimation.ComputeAnimationCurve`en un método o una expresión lambda, el valor devuelto sería el valor pasado en el parámetro Progress.
...
En el caso de los eventos que toman más de un parámetro (en Objective-C, la Convención es que el primer parámetro de una clase de delegado es la instancia del objeto de remitente) debe proporcionar el nombre que desea para la clase EventArgs generada. Esto se hace con el atributo `[EventArgs]` en la declaración del método en la clase `Model`.
...
La declaración anterior generará una `UIImagePickerImagePickedEventArgs` clase que se deriva de EventArgs y empaqueta ambos parámetros, el `UIImage` y el `NSDictionary`. El generador produce lo siguiente:
...
Por ejemplo, el `UIWebView` utiliza el siguiente bit del `UIWebViewDelegate`:
...
El anterior expone `LoadingFinished` como el método en la `UIWebViewDelegate`, pero `LoadFinished` como el evento que se va a enlazar a en un `UIWebView`:
...
Cuando se aplica el atributo `[Model]` a una definición de tipo en la API de contrato, el tiempo de ejecución generará código especial que solo mostrará las invocaciones a los métodos de la clase si el usuario ha sobrescrito un método en la clase. Este atributo se aplica normalmente a todas las API que encapsulan una clase de delegado de Objective-C.
...
Algunos protocolos se usan generalmente como clases de modelo, que se deben enlazar mediante el atributo [`[Model]`](#ModelAttribute) .
...
A partir de Xamarin. iOS 7,0 se ha incorporado una funcionalidad de enlace de protocolo nueva y mejorada.  Cualquier definición que contenga el atributo `[Protocol]` generará tres clases compatibles que mejoran en gran medida la forma en que se consumen los protocolos:
...
El atributo `[Appearance]` está limitado a iOS 5, donde se presentó el administrador de apariencia.

El atributo `[Appearance]` se puede aplicar a cualquier método o propiedad que participe en el marco de `UIAppearance`. Cuando este atributo se aplica a un método o una propiedad en una clase, dirigirá el generador de enlaces para crear una clase de aspecto fuertemente tipada que se utiliza para aplicar estilo a todas las instancias de esta clase o las instancias que coinciden con determinados criterios.
...
Utilice la `[AutoReleaseAttribute]` en métodos y propiedades para encapsular la invocación del método al método en un `NSAutoReleasePool`.

En Objective-C hay algunos métodos que devuelven valores que se agregan a la `NSAutoReleasePool`predeterminada. De forma predeterminada, estas Irán al subproceso `NSAutoReleasePool`, pero como Xamarin. iOS también mantiene una referencia a los objetos mientras el objeto administrado reside, es posible que no desee conservar una referencia adicional en el `NSAutoReleasePool` que solo se purgará hasta que el subproceso vuelva. control al subproceso siguiente o vuelva al bucle principal.

Este atributo se aplica, por ejemplo, en propiedades pesadas (por ejemplo `UIImage.FromFile`) que devuelve los objetos que se han agregado a la `NSAutoReleasePool`predeterminada. Sin este atributo, las imágenes se conservarán siempre que el subproceso no devuelva el control al bucle principal. La conexión del subproceso era algún tipo de descargador en segundo plano que siempre está activo y en espera de trabajo. las imágenes nunca se liberan.

### <a name="forcedtypeattribute"></a>ForcedTypeAttribute

El `[ForcedTypeAttribute]` se utiliza para exigir la creación de un tipo administrado incluso si el objeto no administrado devuelto no coincide con el tipo descrito en la definición de enlace.
...
Indica claramente que devolverá una instancia de `NSURLSessionDownloadTask`, pero, sin embargo, **devuelve** una `NSURLSessionTask`, que es una superclase y, por tanto, no se pueden convertir a `NSURLSessionDownloadTask`. Dado que estamos en un contexto con seguridad de tipos, se producirá un `InvalidCastException`.

Para cumplir con la descripción del encabezado y evitar el `InvalidCastException`, se usa el `[ForcedTypeAttribute]`.
...
El `[ForcedTypeAttribute]` también acepta un valor booleano denominado `Owns` `false` de forma predeterminada `[ForcedType (owns: true)]`. El parámetro conpropiedads se usa para seguir la [Directiva de propiedad](https://developer.apple.com/library/content/documentation/CoreFoundation/Conceptual/CFMemoryMgmt/Concepts/Ownership.html) de los objetos de **Core Foundation** .

El `[ForcedTypeAttribute]` solo es válido en parámetros, propiedades y valores devueltos.

<a name="BindAsAttribute" />

### <a name="bindasattribute"></a>BindAsAttribute

El `[BindAsAttribute]` permite enlazar `NSNumber`, `NSValue` y `NSString`(enumeraciones) en tipos C# más precisos. El atributo se puede usar para crear una API de .NET mejor y precisa a través de la API nativa.

Puede decorar métodos (en el valor devuelto), parámetros y propiedades con `BindAs`. La única restricción es que el miembro **no debe** estar dentro de una interfaz `[Protocol]` o [`[Model]`](#ModelAttribute) .
...
Internamente, se realizarán las conversiones `bool?` <-> `NSNumber` y `CGRect` <-> .
...
Se admite C# la encapsulación de los siguientes tipos de datos en`NSValue`:
...
Se admite C# la encapsulación de los siguientes tipos de datos en`NSNumber`:
...
Administraremos el `enum` <-> `NSString` conversión solo si el tipo de enumeración proporcionado a [`[BindAs]`](#BindAsAttribute) está [respaldado por una constante NSString](#enum-attributes).

#### <a name="arrays"></a>Matrices

[`[BindAs]`](#BindAsAttribute) también admite matrices de cualquiera de los tipos admitidos, puede tener la siguiente definición de API como ejemplo:
...
El parámetro `rects` se encapsulará en una `NSArray` que contenga un `NSValue` para cada `CGRect` y, en la devolución, obtendrá una matriz de `CAScroll?` que se ha creado con los valores de la `NSArray` devuelta que contiene `NSStrings`.

<a name="BindAttribute" />

### <a name="bindattribute"></a>BindAttribute

El atributo `[Bind]` tiene dos usos cuando se aplica a una declaración de método o propiedad y otro cuando se aplica al captador o establecedor individual en una propiedad.

Cuando se usa para un método o una propiedad, el efecto del atributo `[Bind]` es generar un método que invoque el selector especificado. Pero el método generado resultante no se decora con el atributo [`[Export]`](#ExportAttribute) , lo que significa que no puede participar en el reemplazo del método. Normalmente se usa en combinación con el atributo `[Target]` para implementar métodos de extensión Objective-C.
...
Cuando se usa en un captador o un establecedor, el atributo `[Bind]` se usa para modificar los valores predeterminados que el generador de código infiere al generar los nombres de selector de Objective-C del captador y establecedor para una propiedad. De forma predeterminada, cuando se marca una propiedad con el nombre `fooBar`, el generador genera una exportación de `fooBar` para el captador y el `setFooBar:` para el establecedor. En algunos casos, Objective-C no sigue esta Convención, normalmente cambian el nombre del captador para que se `isFooBar`.
...
Lo siguiente generará este método asincrónico, donde `FileLoading` contiene propiedades para tener acceso a `files` y `byteCount`:

```csharp
Task<FileLoading> LoadFile (string file);
```

Si el último parámetro de la devolución de llamada es un `NSError`, el método de `Async` generado comprobará si el valor no es NULL y, en ese caso, el método asincrónico generado establecerá la excepción de la tarea.
...
Y, si se produce un error, la tarea resultante tendrá la excepción establecida en una `NSErrorException` que contiene el `NSError`resultante.

#### <a name="asyncattributeresulttype"></a>AsyncAttribute. ResultType

Utilice esta propiedad para especificar el valor de la devolución de `Task` objeto.   Este parámetro toma un tipo existente, por lo que debe definirse en una de las definiciones de API principales.

#### <a name="asyncattributeresulttypename"></a>AsyncAttribute.ResultTypeName

Utilice esta propiedad para especificar el valor de la devolución de `Task` objeto.   Este parámetro toma el nombre del tipo deseado, el generador generará una serie de propiedades, una para cada parámetro que la devolución de llamada toma.
...
Cuando este atributo se aplica a un constructor, generará el mismo `[DesignatedInitializer]` en el ensamblado de plataforma final. Esto sirve para ayudar al IDE a indicar qué constructor se debe utilizar en las subclases.

Se debe asignar al uso de `__attribute__((objc_designated_initializer))`de Objective-C/Clang.
...
Este atributo solo es necesario en las cadenas si se indica al generador que use el cálculo de referencias de cadenas de copia cero mediante la opción de línea de comandos `--zero-copy` o estableciendo el atributo de nivel de ensamblado `ZeroCopyStringsAttribute`.

Esto es necesario en los casos en los que la propiedad se declara en Objective-C para ser una propiedad `retain` o `assign` en lugar de una propiedad `copy`. Normalmente se producen en bibliotecas de terceros que los desarrolladores han "optimizado" erróneamente. En general, `retain` o `assign` propiedades de `NSString` son incorrectas, ya que `NSMutableString` o las clases derivadas del usuario de `NSString` pueden modificar el contenido de las cadenas sin el conocimiento del código de la biblioteca, con lo que se interrumpe ligeramente la aplicación. Normalmente esto sucede debido a una optimización prematura.
...
Al aplicar el `[DisposeAttribute]` a una clase, se proporciona un fragmento de código que se agregará a la implementación del método `Dispose()` de la clase.

Dado que las herramientas `bmac-native` y `btouch-native` generan automáticamente el método `Dispose`, debe usar el atributo `[Dispose]` para insertar código en la implementación del método `Dispose` generado.
...
El `symbolName` es el símbolo de C con el que se va a vincular. De forma predeterminada, se cargará desde una biblioteca cuyo nombre se deduce del espacio de nombres en el que se define el tipo. Si no es la biblioteca en la que se busca el símbolo, debe pasar el parámetro `libraryName`. Si va a vincular una biblioteca estática, use `__Internal` como parámetro `libraryName`.
...
Este atributo marca el campo de respaldo de una propiedad que se va a anotar con el atributo de `[ThreadStatic]` .NET. Esto resulta útil si el campo es una variable estática de subproceso.

### <a name="marshalnativeexceptions-xamarinios-606"></a>MarshalNativeExceptions (Xamarin. iOS 6.0.6)

Este atributo hará que un método admita excepciones nativas (Objective-C).
En lugar de llamar a `objc_msgSend` directamente, la invocación pasará a través de un Trampoline personalizado que detecta las excepciones de ObjectiveC y las serializa en excepciones administradas.

Actualmente solo se admiten algunas firmas de `objc_msgSend` (se averigua si no se admite una firma cuando se produce un error en la vinculación nativa de una aplicación que usa el enlace con un símbolo *_objc_msgSend* de monotouch_ que falta), pero se puede agregar más en la solicitud.

### <a name="newattribute"></a>NewAttribute

Este atributo se aplica a los métodos y propiedades para que el generador genere la palabra clave `new` delante de la declaración.
...
Este atributo se puede usar sin argumentos para las notificaciones que no tienen ninguna carga útil, o puede especificar una `System.Type` que haga referencia a otra interfaz en la definición de la API, normalmente con el nombre que termina con "EventArgs". El generador convertirá la interfaz en una clase que subclase `EventArgs` e incluirá todas las propiedades que se enumeran allí. El atributo [`[Export]`](#ExportAttribute) debe usarse en la clase `EventArgs` para mostrar el nombre de la clave utilizada para buscar el Diccionario de Objective-C para capturar el valor.
...
O para establecer un objeto específico que se va a observar. Si pasa `null` a `objectToObserve` este método se comportará igual que el otro elemento del mismo nivel.
...
En algunos casos, no hay ninguna constante asociada al valor pasado en el diccionario.  A veces Apple utiliza constantes de símbolos públicos y a veces utiliza constantes de cadena.  De forma predeterminada, el atributo [`[Export]`](#ExportAttribute) de la clase `EventArgs` proporcionada usará el nombre especificado como símbolo público que se va a buscar en tiempo de ejecución.  Si no es así, se supone que se debe buscar como una constante de cadena y, a continuación, pasar el valor `ArgumentSemantic.Assign` al atributo export.
...
Si el tipo de referencia no tiene este atributo, la herramienta de enlace generará una comprobación del valor que se va a asignar antes de pasarlo a Objective-C y generará una comprobación que producirá una `ArgumentNullException` si el valor asignado es `null`.
...
Utilice este atributo para indicar al generador de enlaces que el enlace para este método concreto se debe marcar con una palabra clave `override`.
...
En este caso, la propiedad `Dependencies` se invocará después de agregar o quitar las dependencias del objeto `NSOperation`, asegurándose de que tenemos un gráfico que representa los objetos cargados reales, evitando las pérdidas de memoria, así como los daños en la memoria.
...
Este atributo se aplica a los valores devueltos para marcarlos como objetos proxy. Algunas API de Objective-C devuelven objetos proxy que no se pueden diferenciar de los enlaces de usuario. El efecto de este atributo es marcar el objeto como un objeto `DirectBinding`. Para un escenario de Xamarin. Mac, puede ver la [explicación sobre este error](https://bugzilla.novell.com/show_bug.cgi?id=670844).
...
Si el valor de `doAdd` es true, el parámetro se agrega a la `__mt_{0}_var List<NSObject>;`. Donde se reemplaza `{0}` por el `listName`especificado. Debe declarar este campo de respaldo en la clase parcial complementaria a la API.
...
Cuando el atributo `[Static]` se aplica a un método o propiedad, genera una propiedad o un método estático. Si no se especifica este atributo, el generador genera una propiedad o un método de instancia.
...
En el diseño de los enlaces de Xamarin. iOS/Xamarin. Mac, el atributo `[Wrap]` se usa para encapsular un objeto débilmente tipado con un objeto fuertemente tipado. Esto entra en juego principalmente con objetos de delegado de Objective-C que se declaran normalmente como de tipo `id` o `NSObject`. La Convención usada por Xamarin. iOS y Xamarin. Mac es exponer esos delegados u orígenes de datos como de tipo `NSObject` y se denominan mediante la Convención "Weak" + el nombre que se está exponiendo. Una propiedad `id delegate` de Objective-C se expondría como una propiedad `NSObject WeakDelegate { get; set; }` en el archivo de contrato de la API.

Pero normalmente, el valor que se asigna a este delegado es de un tipo seguro, por lo que se crea el tipo seguro y se aplica el atributo `[Wrap]`, lo que significa que los usuarios pueden optar por usar tipos débiles si necesitan un control adecuado o si necesitan recurrir a trucos de bajo nivel. , o pueden utilizar la propiedad fuertemente tipada para la mayor parte de su trabajo.
...
Y este es el modo en que el usuario usaría la versión fuertemente tipada, tenga en cuenta que el usuario C#aprovecha el sistema de tipos de y usa la palabra clave override para declarar su intención y que no tiene que decorar manualmente el método con `[Export]`, como hicimos eso en el enlace del usuario:
...
Cuando se aplica el atributo `[Wrap]` en un método dentro de un tipo decorado con un atributo `[Category]`, es necesario incluir `This` como primer argumento desde que se genera un método de extensión. Por ejemplo:
...
Los miembros generados por `[Wrap]` no se `virtual` de forma predeterminada; si necesita un miembro de `virtual`, puede establecer en `true` el parámetro de `isVirtual` opcional.
...
`[Wrap]` también se pueden usar directamente en captadores y establecedores de propiedad.
...
Puede usar el atributo `[Params]` en el último parámetro de matriz de una definición de método para que el generador Inserte un "PARAM" en la definición.   Esto permite que el enlace permita fácilmente parámetros opcionales.
...
Puede usar el atributo `[PlainString]` delante de los parámetros de cadena para indicar al generador de enlaces que pase la cadena como una cadena de C, en lugar de pasar el parámetro como un `NSString`.

La mayoría de las API de Objective-C consumen `NSString` parámetros, pero unas cuantas API exponen una API `char *` para pasar cadenas, en lugar de la variación `NSString`.
Use `[PlainString]` en estos casos.
...
Indica al generador que mantenga una referencia al parámetro especificado. El generador proporcionará la memoria auxiliar para este campo o puede especificar un nombre (el `WrapName`) para almacenar el valor en. Esto resulta útil para contener una referencia a un objeto administrado que se pasa como parámetro a Objective-C y cuando se sabe que Objective-C solo conservará esta copia del objeto. Por ejemplo, una API como `SetDisplay (SomeObject)` usaría este atributo, ya que es probable que SetDisplay solo muestre un objeto cada vez. Si necesita realizar un seguimiento de más de un objeto (por ejemplo, para una API de tipo pila), usaría el atributo `[RetainList]`.
...
Este atributo se aplica a los parámetros y solo se usa cuando se realiza la transición de Objective C#-C a.  Durante esas transiciones, los distintos parámetros de `NSObject` de Objective-C se ajustan en una representación administrada del objeto.
...
La asignación de constantes de `NSString` a valores de enumeración es una manera fácil de crear una API de .NET. Instalar

* permite que la finalización del código sea más útil, mostrando **solo** los valores correctos de la API;
* agrega seguridad de tipos, no se puede utilizar otra `NSString` constante en un contexto incorrecto; etc
...
En la definición de enlace anterior, el generador creará el `enum` mismo y también creará un `*Extensions` tipo estático que incluye métodos de conversión de dos maneras entre los valores de enumeración y las constantes de `NSString`. Esto significa que las constantes siguen estando disponibles para los desarrolladores aunque no formen parte de la API.
...
Si no se decora ningún valor de enumeración, se producirá una `NotSupportedException`.
...
Es el mismo `[Field]` atributo que se usa para las constantes dentro del tipo. También se puede utilizar dentro de las enumeraciones para asignar un valor a una constante específica.

Se puede utilizar un valor `null` para especificar el valor de enumeración que se debe devolver si se especifica una constante de `NSString` `null`.
...
Si no hay ningún valor de `null` presente, se producirá una `ArgumentNullException`.

## <a name="global-attributes"></a>Atributos globales

Los atributos globales se aplican mediante el modificador de `[assembly:]` atributo como el [`[LinkWithAttribute]`](#LinkWithAttribute) o se pueden usar en cualquier lugar, como los atributos [`[Lion]`](#SinceAndLionAttributes) y [`[Since]`](#SinceAndLionAttributes) .
...
Cuando se usa el atributo `[LinkWith]`, el `libraryName` especificado se incrusta en el ensamblado resultante, lo que permite a los usuarios enviar un único archivo DLL que contiene las dependencias no administradas, así como las marcas de línea de comandos necesarias para utilizar correctamente la biblioteca de. Xamarin. iOS.

También es posible no proporcionar un `libraryName`, en cuyo caso el atributo de `LinkWith` se puede usar para especificar solo marcas de enlazador adicionales:
...
Tenga en cuenta que Xamarin. iOS infiere el argumento `LinkTarget` y no es necesario establecerlo.
...
La propiedad `ForceLoad` se utiliza para decidir si se utiliza o no la marca de vínculo de `-force_load` para vincular la biblioteca nativa. Por ahora, siempre debe ser true.

#### <a name="linkwithattributeframeworks"></a>LinkWithAttribute. frameworks

Si la biblioteca que se está enlazando tiene un requisito estricto en cualquier marco de trabajo (que no sea `Foundation` y `UIKit`), debe establecer la propiedad `Frameworks` en una cadena que contenga una lista delimitada por espacios de los marcos de plataforma necesarios. Por ejemplo, si va a enlazar una biblioteca que requiere `CoreGraphics` y `CoreText`, debe establecer la propiedad `Frameworks` en `"CoreGraphics CoreText"`.
...
Las versiones anteriores de Xamarin. iOS comprobó la propiedad `LinkTarget` para determinar la plataforma admitida por la biblioteca, pero ahora se detecta automáticamente y se omite la propiedad `LinkTarget`.

#### <a name="linkwithattributelinkerflags"></a>LinkWithAttribute.LinkerFlags

La cadena de `LinkerFlags` proporciona una manera de enlazar a los autores de especificar cualquier marcador de enlazador adicional necesario al vincular la biblioteca nativa a la aplicación.

Por ejemplo, si la biblioteca nativa requiere libxml2 y zlib, establezca la cadena de `LinkerFlags` en `"-lxml2 -lz"`.
...
La propiedad `SmartLink` debe establecerse en true para permitir que Xamarin. iOS determine si `ForceLoad` es obligatorio o no.

#### <a name="linkwithattributeweakframeworks"></a>LinkWithAttribute.WeakFrameworks

La propiedad `WeakFrameworks` funciona de la misma forma que la propiedad `Frameworks`, salvo que, en tiempo de vínculo, el especificador de `-weak_framework` se pasa a GCC para cada uno de los marcos de trabajo de la lista.

`WeakFrameworks` permite que las bibliotecas y aplicaciones se vinculen de forma débil con los marcos de la plataforma para que puedan usarlos opcionalmente si están disponibles, pero no tienen una dependencia fuerte en ellos, lo que resulta útil si la biblioteca está pensada para agregar características adicionales en. versiones más recientes de iOS. Para obtener más información sobre la vinculación débil, consulte la documentación de Apple sobre la [vinculación débil](https://developer.apple.com/library/mac/#documentation/MacOSX/Conceptual/BPFrameworks/Concepts/WeakLinking.html).

Los buenos candidatos para la vinculación débil serían `Frameworks` como cuentas, `CoreBluetooth`, `CoreImage`, `GLKit`, `NewsstandKit` y `Twitter`, ya que solo están disponibles en iOS 5.
...
El atributo `[Since]` se usa para marcar las API como si se hubieran introducido en un momento determinado. El atributo solo se debe usar para marcar tipos y métodos que podrían provocar un problema en tiempo de ejecución si la clase, el método o la propiedad subyacentes no están disponibles.
...
Se trata de una subclase especializada del atributo `[Advice]` que se puede usar para sugerir al desarrollador que invalide un método **requiere** una llamada al método base (invalidado).
...
Este atributo indica al generador que el enlace para esta biblioteca específica (si se aplica con `[assembly:]`) o el tipo debe utilizar la serialización de cadena de copia rápida de cero. Este atributo equivale a pasar la opción de línea de comandos `--zero-copy` al generador.

Cuando se usa la copia de cero para las cadenas, el generador utiliza C# de forma eficaz la misma cadena que la cadena que Objective-c consume sin incurrir en la creación de un nuevo objeto de`NSString`y C# evitando copiar los datos de las cadenas en Objective-c String@. El único inconveniente de usar cero cadenas de copia es que debe asegurarse de que cualquier propiedad de cadena que se ajuste se marque como `retain` o `copy` tenga el atributo `[DisableZeroCopy]` establecido. Esto es necesario porque el identificador de las cadenas de copia cero se asigna en la pila y no es válido en la función que devuelve.
...
Con Xamarin. iOS 8,0 hemos incorporado compatibilidad para crear fácilmente clases fuertemente tipadas que contienen `NSDictionaries`.
...
Esto genera automáticamente una clase de la que se pueden crear instancias desde un `NSDictionary` existente o que se ha creado.
...
En el caso anterior, la clase `MyOption` producirá una propiedad de cadena para `Name` que utilizará la `MyOptionKeys.NameKey` como clave en el diccionario para recuperar una cadena.   Y usarán el `MyOptionKeys.AgeKey` como clave en el diccionario para recuperar una `NSNumber` que contiene un int.
...
En la definición de `StrongDictionary` se admiten los siguientes tipos de datos:

|C#Tipo de interfaz|`NSDictionary` tipo de almacenamiento|
|---|---|
|`bool`|`Boolean` almacenados en una `NSNumber`|
...
|`nint`|`NSInteger` almacenados en una `NSNumber`|
|`nuint`|`NSUInteger` almacenados en una `NSNumber`|
