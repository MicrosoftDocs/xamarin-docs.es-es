...
[![](objective-c-libraries-images/00-sml.png "Para ello, seleccione el tipo Visual Studio para Mac de proyecto, biblioteca de enlaces de la biblioteca de iOS")](objective-c-libraries-images/00.png#lightbox)
...
[![](objective-c-libraries-images/00vs-sml.png "Biblioteca de enlaces de iOS iOS")](objective-c-libraries-images/00vs.png#lightbox)
...
En el ejemplo anterior se define una `Cocos2D.Camera` clase denominada que se deriva del tipo `Foundation.NSObject``ZEye` base(estetipoprocedede)yquedefineunapropiedadestática(),dosmétodosquenotomanargumentosyun`NSObject` método que toma tres argumentos.
...
Cualquier código adicional que necesite como enumeraciones o clases auxiliares se debe hospedar en un archivo independiente, en el ejemplo anterior, "CameraMode" es un valor de enumeración que no existe en el archivo CS y que se debe hospedar en un archivo independiente `StructsAndEnums.cs` , por ejemplo :
...
El `APIDefinition.cs` archivo se combina con la `StructsAndEnum` clase y se utiliza para generar el enlace básico de la biblioteca. Puede usar la biblioteca resultante tal cual, pero por lo general, querrá optimizar la biblioteca resultante para agregar algunas C# características para las ventajas de los usuarios. Entre los ejemplos se incluye `ToString()` la implementación de C# un método, la inclusión de indexadores, la adición de conversiones implícitas a y desde algunos tipos nativos o la entrega de versiones fuertemente tipadas de algunos métodos. Estas mejoras se almacenan en C# archivos adicionales. Simplemente agregue los C# archivos al proyecto y se incluirán en este proceso de compilación.

Esto muestra cómo se implementaría el código en el `Extra.cs` archivo. Tenga en cuenta que usará clases parciales a medida que aumentan las clases parciales que se generan a partir de la combinación `ApiDefinition.cs` de y `StructsAndEnums.cs` el enlace básico:
...
Para completar este enlace, debe agregar la biblioteca nativa al proyecto.  Puede hacerlo agregando la biblioteca nativa al proyecto, arrastrando y colocando la biblioteca nativa desde Finder en el proyecto en el explorador de soluciones, o haciendo clic con el botón derecho en el proyecto y eligiendo **Agregar** > **Agregar archivos** a Seleccione la biblioteca nativa.
Las bibliotecas nativas por Convención comienzan con la palabra "lib" y terminan con la extensión ". a". Al hacerlo, Visual Studio para Mac agregará dos archivos: el archivo. a y un archivo rellenado C# automáticamente que contiene información acerca de lo que contiene la biblioteca nativa:
 ...
 [![](objective-c-libraries-images/screen-shot-2012-02-08-at-3.45.06-pm.png "Las bibliotecas nativas por Convención comienzan con la palabra lib y terminan con la extensión.")](objective-c-libraries-images/screen-shot-2012-02-08-at-3.45.06-pm.png#lightbox)
...
Ahora, al compilar el proyecto, terminará con `MagicChords.dll` un archivo que contiene el enlace y la biblioteca nativa. Puede distribuir este proyecto o el archivo DLL resultante a otros desarrolladores para su propio uso.
...
El enlace más sencillo que puede hacer es enlazar un método. Basta con declarar un método en la interfaz con C# las convenciones de nomenclatura y decorar el método con el[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute)
atribui. El [`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute) atributo es lo que vincula C# su nombre con el nombre de Objective-C en el tiempo de ejecución de Xamarin. iOS. Parámetro de la[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute) 
...
En los ejemplos anteriores se muestra cómo se pueden enlazar métodos de instancia. Para enlazar métodos estáticos, debe usar [`[Static]`](~/cross-platform/macios/binding/binding-types-reference.md#StaticAttribute) el atributo, de la siguiente manera:
...
Al exportar un tipo de referencia, con [`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute) la palabra clave también puede especificar la semántica de asignación. Esto es necesario para garantizar que no se pierde ningún dato.
...
Donde el captador y el establecedor se definen explícitamente `setName` como en los `name` enlaces y anteriores.

Además de admitir las propiedades estáticas mediante [`[Static]`](~/cross-platform/macios/binding/binding-types-reference.md#StaticAttribute), puede decorar las propiedades estáticas de [`[IsThreadStatic]`](~/cross-platform/macios/binding/binding-types-reference.md#IsThreadStaticAttribute)los subprocesos con, por ejemplo:
...
1. **Las propiedades de enlace deben ser estáticas** : al definir el enlace de [`[Static]`](~/cross-platform/macios/binding/binding-types-reference.md#StaticAttribute) propiedades, se debe usar el atributo.
...
Los marcos de trabajo de Objective-C usan una expresión en la que algunas clases son inmutables con una subclase mutable. Por ejemplo `NSString` , la versión inmutable, `NSMutableString` mientras que es la subclase que permite la mutación.
...
La forma en que se asigna a C# esto es agregando el captador y el establecedor en la clase base, pero marcando el establecedor con un[`[NotImplemented]`](~/cross-platform/macios/binding/binding-types-reference.md#NotImplementedAttribute)
atribui.
...
Por ejemplo, considere `UIKit` la `UIAccelerometerDelegate`clase, así es como se enlaza en Xamarin. iOS:
...
Dado que se trata de un método opcional en la `UIAccelerometerDelegate` definición de, no hay nada más que hacer. Pero si hubiera un método necesario en el protocolo, debes agregar el[`[Abstract]`](~/cross-platform/macios/binding/binding-types-reference.md#AbstractAttribute)
...
La Convención en Xamarin. iOS es admitir el estilo de acoplamiento flexible de Objective-C en el que se puede asignar cualquier `NSObject` instancia de un al delegado, y también exponer una versión fuertemente tipada. Por esta razón, normalmente se proporciona una `Delegate` propiedad fuertemente tipada y una que tiene un `WeakDelegate` tipo flexible. Normalmente enlazamos la versión con tipo flexible con [`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute)y usamos el [`[Wrap]`](~/cross-platform/macios/binding/binding-types-reference.md#WrapAttribute) atributo para proporcionar la versión fuertemente tipada.
...
Para cada definición `MyProtocol` de protocolo en Objective-C, ahora hay una `IMyProtocol` interfaz que enumera todos los métodos necesarios del Protocolo, así como una clase de extensión que proporciona todos los métodos opcionales.  Lo anterior, combinado con la nueva compatibilidad en el editor de Xamarin Studio permite a los desarrolladores implementar métodos de protocolo sin tener que usar las subclases independientes de las clases de modelo abstracto anteriores.
...
En Objective-C es posible extender clases con nuevos métodos, similares en el espíritu de los C#métodos de extensión de. Cuando uno de estos métodos esté presente, puede usar el[`[BaseType]`](~/cross-platform/macios/binding/binding-types-reference.md#BaseTypeAttribute) 
atributo para marcar el método como receptor del mensaje de Objective-C.

Por ejemplo, en Xamarin. iOS enlazamos los métodos de extensión que se definen `NSString` en `UIKit` cuando se importa como métodos en `NSStringDrawingExtensions`, de la siguiente manera:
...
Para enlazar un campo, agregue una propiedad al archivo de definición de interfaz y decore la propiedad [`[Field]`](~/cross-platform/macios/binding/binding-types-reference.md#FieldAttribute) con el atributo. Este atributo toma un parámetro: el nombre de C del símbolo que se va a buscar. Por ejemplo:
...
Lo anterior generará un `LonelyClass` que no deriva de `NSObject` y contendrá un enlace `NSString`al `NSSomeEventNotification` 
 `NSString` expuesto como.
...
Sin embargo, esto limitaría la subclase del tipo, ya que la alternativa de [`[Wrap]`](~/cross-platform/macios/binding/binding-types-reference.md#WrapAttribute) API más agradable utiliza un atributo. Los métodos generados no `virtual`son, es decir, no podrá invalidarlos, lo que puede ser una buena opción.
...
### <a name="binding-nsvalue-nsnumber-and-nsstring-to-a-better-type"></a>`NSValue`Enlazar `NSNumber`, y`NSString` a un tipo mejor

El [`[BindAs]`](~/cross-platform/macios/binding/binding-types-reference.md#BindAsAttribute) atributo permite `NSNumber` enlazar`NSString`y (enumeraciones) en tipos más precisos C# `NSValue` El atributo se puede usar para crear una API de .NET mejor y precisa a través de la API nativa.

Puede decorar métodos (en el valor devuelto), parámetros y [`[BindAs]`](~/cross-platform/macios/binding/binding-types-reference.md#BindAsAttribute)propiedades con. La única restricción es que el miembro **no debe** estar dentro de un[`[Protocol]`](~/cross-platform/macios/binding/binding-types-reference.md#ProtocolAttribute) 
o [`[Model]`](~/cross-platform/macios/binding/binding-types-reference.md#ModelAttribute) la interfaz.
...
Este atributo se puede usar sin argumentos para las notificaciones que no tienen ninguna carga útil, o puede `System.Type` especificar un que haga referencia a otra interfaz en la definición de la API, normalmente con el nombre que termina con "EventArgs". El generador convertirá la interfaz en una clase que crea subclases `EventArgs` e incluirá todas las propiedades que aparecen en la lista. El [`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute) atributo debe usarse en la clase EventArgs para mostrar el nombre de la clave utilizada para buscar el Diccionario de Objective-C para capturar el valor.
...
Lo anterior generará una `MyScreenChangedEventArgs` clase con las `ScreenX` propiedades `ScreenY` y que recuperarán los datos del diccionario [NSNotification. UserInfo](xref:Foundation.NSNotification.UserInfo) usando las claves "ScreenXKey" y "ScreenYKey", respectivamente, y aplicarán el conversiones. El `[ProbePresence]` atributo se usa para que el generador sondee si la clave se establece `UserInfo`en, en lugar de intentar extraer el valor. Se utiliza para los casos en los que la presencia de la clave es el valor (normalmente para los valores booleanos).
...
Las categorías son un mecanismo de Objective-C que se usa para ampliar el conjunto de métodos y propiedades disponibles en una clase.   En la práctica, se usan para extender la funcionalidad de una clase base (por ejemplo `NSObject`,) cuando se vincula un marco de trabajo específico en (por ejemplo `UIKit`,), lo que permite que sus métodos estén disponibles, pero solo si el nuevo marco de trabajo está vinculado en.   En algunos otros casos, se usan para organizar características en una clase por funcionalidad.   Son similares en lo que se C# refiere a los métodos de extensión. Este es el aspecto que tendría una categoría en Objective-C:
...
En el ejemplo anterior, si se encuentra en una biblioteca, `UIView` se extienden `makeBackgroundRed`las instancias de con el método.
...
Lo anterior creará una `MyUIViewExtension` clase que contiene el método `MakeBackgroundRed` de extensión.  Esto significa que ahora puede llamar a "MakeBackgroundRed" en cualquier `UIView` subclase, lo que le proporciona la misma funcionalidad que obtendría en Objective-C. En algunos otros casos, las categorías se utilizan para no extender una clase de sistema, sino para organizar la funcionalidad exclusivamente con fines de decoración.  Así:
...
Los bloques son una nueva construcción presentada por Apple para poner el equivalente funcional C# de métodos anónimos a Objective-C. Por ejemplo, la `NSSet` clase ahora expone este método:

```csharp
- (void) enumerateObjectsUsingBlock:(void (^)(id obj, BOOL *stop) block
```

La descripción anterior declara un método denominado `enumerateObjectsUsingBlock:` que toma un argumento denominado. `block` Este bloque es similar a un C# método anónimo, ya que admite la captura del entorno actual (el puntero "this", el acceso a las variables locales y los parámetros). El método anterior de `NSSet` invoca el bloque con dos `NSObject` parámetros (la `id obj` parte) y un puntero a un elemento booleano (el `BOOL *stop`).
...
En muchos lugares de la API de Objective-C, los parámetros se pasan como `NSDictionary` API de tipo débil con claves y valores específicos, pero son propensos a errores (puede pasar claves no válidas y no obtener ninguna advertencia; puede pasar valores no válidos y no obtener ninguna advertencia) y frustrante para usar, ya que requieren varios viajes a la documentación para buscar los nombres y valores de clave posibles.
...
Por ejemplo, si la API de Objective-C aceptó `NSDictionary` un y se documenta como si tomara `XyzVolumeKey` la clave que `NSNumber` toma un con un valor de volumen de 0,0 a `XyzCaptionKey` 1,0 y un que toma una cadena, querrá que los usuarios tengan una buena API tiene el siguiente aspecto:
...
atributo para mostrar un nuevo punto de entrada de API y lo encontramos con nuestra `XyzOptions` clase fuertemente tipada.  El método contenedor también permite pasar valores NULL.

Ahora, lo que no hemos mencionado es de dónde proceden `XyzOptionsKeys` los valores.  Normalmente agruparía las claves que una API expone en una clase estática como `XyzOptionsKeys`, como se muestra a continuación:
...
En caso de que necesite hacer referencia a `XyzOption` los miembros en un campo diferente (que no es el nombre de la propiedad con `Key`el sufijo), puede decorar la propiedad con un[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute) 
...
El tiempo de ejecución de Xamarin. iOS se encarga C# automáticamente de convertir `NSArrays` las matrices en y de volver a realizar la conversión, por ejemplo, el método imaginario `NSArray` de `UIViews`Objective-C que devuelve un de:
...
En los casos en los que no se puede realizar un seguimiento del tipo más derivado real incluido en la matriz `NSObject []` , se puede utilizar como valor devuelto.

<a name="Selectors" />

### <a name="selectors"></a>Selectores

Los selectores aparecen en la API de Objective-C como `SEL`el tipo especial. Al enlazar un selector, asignaría el tipo a `ObjCRuntime.Selector`.  Normalmente, los selectores se exponen en una API con un objeto, el objeto de destino y un selector que se va a invocar en el objeto de destino. Proporcionar ambos se corresponde básicamente con el C# delegado: algo que encapsula el método que se va a invocar, así como el objeto en el que se va a invocar el método.
...
Para que el enlace sea más agradable C# a los desarrolladores, normalmente proporcionará un método que toma `NSAction` un parámetro, lo C# que permite usar delegados y expresiones lambda en lugar `Target+Selector`de. Para ello, normalmente ocultaría el `SetTarget` método marcándolos con un[`[Internal]`](~/cross-platform/macios/binding/binding-types-reference.md#InternalAttribute)
...
El único caso en el que es posible que desee `NSString` usar directamente es cuando la cadena se usa como un token. Para obtener más información sobre las `NSString`cadenas y, lea el documento [sobre el diseño de la API en NSString](~/ios/internals/api-design/nsstring.md) .
...
Se utiliza para especificar el nombre de la `EventArgs` clase que se va a generar. `EventArgs` Debe usar una por firma (en este ejemplo, `EventArgs` contendrá una `With` propiedad de tipo NINT).
...
Las devoluciones de llamada son similares a las invocaciones de eventos, la diferencia es que en lugar de tener varios suscriptores potenciales (por ejemplo, varios `Clicked` métodos se pueden `DownloadFinished` enlazar a un evento o un evento) las devoluciones de llamada solo pueden tener un único suscriptor.

El proceso es idéntico, la única diferencia es que en lugar de exponer el nombre de la `EventArgs` clase que se va a generar, el EventArgs realmente se usa para asignar un nombre al nombre de delegado resultante. C#
...
atributos [`[DefaultValueFromArgument]`](~/cross-platform/macios/binding/binding-types-reference.md#DefaultValueFromArgumentAttribute) o.
...
Debe informar a Xamarin. iOS sobre cómo vincular las bibliotecas; esto puede hacerse modificando la configuración de compilación para invocar el `mtouch` comando con algunos argumentos de compilación adicionales que especifican cómo se vinculan las nuevas bibliotecas con la opción "-gcc_flags", seguido de una cadena entrecomillada que contiene todas las bibliotecas adicionales que se necesitan para el programa, como se muestra a continuación:

```bash
-gcc_flags "-L${ProjectDir} -lMylibrary -force_load -lSystemLibrary -framework CFNetwork -ObjC"
```

En el ejemplo anterior se `libMyLibrary.a`vinculará `libSystemLibrary.dylib` y `CFNetwork` la biblioteca de Framework en el ejecutable final.

También puede aprovechar el nivel [`[LinkWithAttribute]`](~/cross-platform/macios/binding/binding-types-reference.md#LinkWithAttribute)de ensamblado, que puede insertar en los archivos de contrato ( `AssemblyInfo.cs`como).
...
Es posible que se pregunte por qué necesita `-force_load` el comando, y el motivo es que la marca-ObjC a pesar de que compila el código en, no conserva los metadatos necesarios para admitir las categorías (la eliminación de código inactivo del vinculador o del compilador lo elimina) que usted necesita en tiempo de ejecución para Xamarin. iOS.
...
A partir de Xamarin. iOS v 3.2, se admite la herencia de los protocolos que se han marcado [`[Model]`](~/cross-platform/macios/binding/binding-types-reference.md#ModelAttribute) con la propiedad. Esto resulta útil en ciertos patrones de API, como en `MapKit` el lugar `MKOverlay` en el que el protocolo hereda `MKAnnotation` del protocolo y lo adoptan varias clases que heredan de `NSObject`.

Históricamente, se necesitaba copiar el protocolo en cada implementación, pero en estos casos podemos hacer que la `MKShape` clase se herede `MKOverlay` del protocolo y generará automáticamente todos los métodos necesarios.
