...
En el código XAML siguiente se muestra cómo crear [`StackLayout`](xref:Xamarin.Forms.StackLayout) un objeto [`Label`](xref:Xamarin.Forms.Label) vertical que contiene tres objetos:
...
En, si el tamaño de un elemento no se establece explícitamente, se expande para rellenar el ancho disponible o el alto [`Orientation`](xref:Xamarin.Forms.StackLayout.Orientation) si la propiedad está [`Horizontal`establecidaen.](xref:Xamarin.Forms.StackOrientation) [`StackLayout`](xref:Xamarin.Forms.StackLayout)
...
Es una pérdida de tiempo porque se realizan cálculos de diseño innecesarios. En su lugar, el diseño deseado puede lograrse mejor mediante el [`Grid`](xref:Xamarin.Forms.Grid)uso de.
...
[`Grid`](xref:Xamarin.Forms.Grid) Se usa para mostrar elementos en filas y columnas, que pueden tener tamaños proporcionales o absolutos. Las filas y columnas de una cuadrícula se especifican [`RowDefinitions`](xref:Xamarin.Forms.Grid.RowDefinitions) con [`ColumnDefinitions`](xref:Xamarin.Forms.Grid.ColumnDefinitions) las propiedades y.

Para colocar los elementos en [`Grid`](xref:Xamarin.Forms.Grid) celdas específicas, use [`Grid.Column`](xref:Xamarin.Forms.Grid.ColumnProperty) las [`Grid.Row`](xref:Xamarin.Forms.Grid.RowProperty) propiedades adjuntas y. Para que los elementos abarquen varias filas y columnas, [`Grid.RowSpan`](xref:Xamarin.Forms.Grid.RowSpanProperty) Utilice [`Grid.ColumnSpan`](xref:Xamarin.Forms.Grid.ColumnSpanProperty) y las propiedades adjuntas.

> [!NOTE]
> Un [`Grid`](xref:Xamarin.Forms.Grid) diseño no se debe confundir con las tablas y no está diseñado para presentar datos tabulares. A diferencia de las tablas HTML `Grid` , una está pensada para diseñar el contenido. Para Mostrar datos tabulares, considere la posibilidad de usar un [control ListView](~/xamarin-forms/user-interface/listview/index.md), [CollectionView](~/xamarin-forms/user-interface/collectionview/index.md)o [TableView](~/xamarin-forms/user-interface/tableview.md).

En el siguiente código XAML se muestra cómo [`Grid`](xref:Xamarin.Forms.Grid) crear un con dos filas y dos columnas:
...
En el código XAML siguiente se muestra cómo [`FlexLayout`](xref:Xamarin.Forms.FlexLayout) crear un que muestra sus vistas en una sola columna:
...
- La [`Direction`](xref:Xamarin.Forms.FlexLayout.Direction) propiedad se establece en `Column`, lo que `FlexLayout` hace que los elementos secundarios de se organicen en una sola columna de elementos.
...
En el código XAML siguiente se muestra cómo organizar los [`RelativeLayout`](xref:Xamarin.Forms.RelativeLayout)elementos en:
...
- El color [`BoxView`](xref:Xamarin.Forms.BoxView) verde no tiene un tamaño explícito. Su posición se establece en relación con `BoxView` el `pole`nombre.

> [!WARNING]
> Evite usar `RelativeLayout` siempre que sea posible. Como resultado, la CPU tendrá que realizar mucho más trabajo.
...
> [!IMPORTANT]
> El `HorizontalOptions` y `VerticalOptions` propiedades no tienen ningún efecto en los elementos secundarios de un `AbsoluteLayout`.

Dentro de [`AbsoluteLayout.LayoutBounds`](xref:Xamarin.Forms.AbsoluteLayout.LayoutBoundsProperty) , la propiedad adjunta se utiliza para especificar la posición horizontal, la posición vertical, el ancho y el alto de un elemento. [`AbsoluteLayout`](xref:Xamarin.Forms.AbsoluteLayout) Además, la [`AbsoluteLayout.LayoutFlags`](xref:Xamarin.Forms.AbsoluteLayout.LayoutFlagsProperty) propiedad adjunta especifica cómo se interpretarán los límites del diseño.

En el siguiente código XAML se muestra cómo organizar los [`AbsoluteLayout`](xref:Xamarin.Forms.AbsoluteLayout)elementos de:
...
- A [`BoxView`](xref:Xamarin.Forms.BoxView) cada una se le asigna un tamaño explícito de 100x100 y se muestra en la misma posición, centrado horizontalmente.
- El color [`BoxView`](xref:Xamarin.Forms.BoxView) rojo gira 30 grados y el verde `BoxView` gira 60 grados.
...
Cada elemento visual tiene una [`InputTransparent`](xref:Xamarin.Forms.VisualElement.InputTransparent) propiedad que se utiliza para definir si el elemento recibe la entrada. Su valor predeterminado es `false`, lo que garantiza que el elemento recibe la entrada.

Cuando esta propiedad se establece en una clase de diseño, su valor se transfiere a los elementos secundarios. Por consiguiente, si [`InputTransparent`](xref:Xamarin.Forms.VisualElement.InputTransparent) se establece `true` la propiedad en en una clase de diseño, todos los elementos del diseño no recibirán una entrada.
