...
Mientras actualiza el manifiesto de **Android**, asegúrese también de que está habilitado el permiso `Internet`.
...
Mientras actualiza el manifiesto de **Android**, asegúrese también de que está habilitado el permiso `INTERNET` (en **permisos necesarios**).
...
Esta instrucción hace que la clase `GoogleApiAvailability` de **Xamarin. GooglePlayServices. base** esté disponible para el código **FCMClient** .
...
3. Instale este paquete en el proyecto **FCMClient** :
    ...
    [![la instalación de la mensajería de Xamarin Firebase](remote-notifications-with-fcm-images/03-firebase-messaging-vs-sml.png)](remote-notifications-with-fcm-images/03-firebase-messaging-vs.png#lightbox)
...
3. Instale este paquete en el proyecto **FCMClient** :
    ...
    [![la instalación de la mensajería de Xamarin Firebase](remote-notifications-with-fcm-images/03-firebase-messaging-xs-sml.png)](remote-notifications-with-fcm-images/03-firebase-messaging-xs.png#lightbox)
...
3. Haga clic con el botón secundario en **Google-Services. JSON**.
...
Esta `TextView` se usará para mostrar mensajes que indiquen si Google Play Services está instalado. Guarde los cambios en **Main. axml**.
...
Las variables `CHANNEL_ID` y `NOTIFICATION_ID` se usarán en el [`CreateNotificationChannel`](#create-notification-channel-code) de método que se agregará a `MainActivity` posteriormente en este tutorial.

En el ejemplo siguiente, el método `OnCreate` comprobará que Google Play Services está disponible antes de que la aplicación intente usar FCM Services.
Agregue el método siguiente a la clase `MainActivity`:
...
Este código comprueba el dispositivo para ver si está instalado el Google Play Services APK. Si no está instalado, se muestra un mensaje en el `TextBox` que indica al usuario que descargue un APK del Google Play Store (o que lo habilite en la configuración del sistema del dispositivo).

<a name="create-notification-channel-code"></a>Las aplicaciones que se ejecutan en Android 8,0 (nivel de API 26) o superior deben crear un [_canal de notificación_](~/android/app-fundamentals/notifications/local-notifications.md) para publicar sus notificaciones.  Agregue el método siguiente a la clase `MainActivity` que creará el canal de notificación (si es necesario):
...
El siguiente paso consiste en agregar un servicio que extienda `FirebaseInstanceIdService` para administrar la creación, la rotación y la actualización de los [tokens de registro de Firebase](~/android/data-cloud/google-messaging/firebase-cloud-messaging.md#fcm-in-action-registration-token). El servicio `FirebaseInstanceIdService` es necesario para que FCM pueda enviar mensajes al dispositivo. Cuando se agrega el servicio `FirebaseInstanceIdService` a la aplicación cliente, la aplicación recibirá automáticamente mensajes FCM y los mostrará como notificaciones cada vez que la aplicación esté en el fondo.

### <a name="declare-the-receiver-in-the-android-manifest"></a>Declarar el receptor en el manifiesto de Android

Edite **archivo AndroidManifest. XML** e inserte los siguientes elementos `<receiver>` en la sección `<application>`:
...
- Declara una implementación de `FirebaseInstanceIdReceiver` que proporciona un [identificador único](https://developers.google.com/instance-id/) para cada instancia de la aplicación. Este receptor también autentica y autoriza las acciones.

- Declara una implementación de `FirebaseInstanceIdInternalReceiver` interna que se usa para iniciar los servicios de forma segura.

- El [identificador](./firebase-cloud-messaging.md#fcm-in-action-app-id) de la aplicación se almacena en el archivo **Google-Services. JSON** que se [agregó al proyecto](#add-googleplayservices-json). Los enlaces de Firebase de Xamarin. Android reemplazarán el token `${applicationId}` por el identificador de la aplicación; no se requiere código adicional para que la aplicación cliente proporcione el identificador de la aplicación.

El `FirebaseInstanceIdReceiver` es un `WakefulBroadcastReceiver` que recibe eventos `FirebaseInstanceId` y `FirebaseMessaging` y los entrega a la clase que deriva de `FirebaseInstanceIdService`.

### <a name="implement-the-firebase-instance-id-service"></a>Implementar el servicio de ID. de instancia de Firebase

El trabajo de registro de la aplicación con FCM se controla mediante el servicio de `FirebaseInstanceIdService` personalizado proporcionado por el usuario.
...
Este servicio implementa un método `OnTokenRefresh` que se invoca cuando el token de registro se crea o cambia inicialmente. Cuando se ejecuta `OnTokenRefresh`, recupera el token más reciente de la propiedad `FirebaseInstanceId.Instance.Token` (que se actualiza de forma asincrónica mediante FCM). En este ejemplo, el token actualizado se registra para que se pueda ver en la ventana de salida:
...
`OnTokenRefresh` también llama a `SendRegistrationToAppServer` para asociar el token de registro del usuario con la cuenta del servidor (si existe) que mantiene la aplicación:
...
Dado que esta implementación depende del diseño del servidor de aplicaciones, en este ejemplo se proporciona un cuerpo de método vacío. Si el servidor de aplicaciones requiere la información de registro de FCM, modifique `SendRegistrationToAppServer` para asociar el token de ID. de instancia de FCM del usuario con cualquier cuenta de servidor mantenida por la aplicación. (Tenga en cuenta que el token es opaco a la aplicación cliente).

Cuando se envía un token al servidor de aplicaciones, `SendRegistrationToAppServer` debe mantener un valor booleano para indicar si el token se ha enviado al servidor. Si este valor booleano es false, `SendRegistrationToAppServer` envía el token al servidor de aplicaciones &ndash; de lo contrario, el token ya se envió al servidor de aplicaciones en una llamada anterior. En algunos casos (como este ejemplo `FCMClient`), el servidor de aplicaciones no necesita el token; por consiguiente, este método no es necesario para este ejemplo.
...
El código agregado en este paso está pensado únicamente para fines de demostración &ndash; una aplicación cliente de producción no tendría necesidad de registrar tokens de registro. Edite **Resources/layout/main. axml** y agregue la siguiente declaración de `Button` inmediatamente después del elemento `TextView`:
...
Cuando el usuario pulsa una notificación emitida desde **FCMClient**, todos los datos que acompañan a ese mensaje de notificación estarán disponibles en `Intent` extras. Edite **MainActivity.CS** y agregue el código siguiente en la parte superior del método `OnCreate` (antes de la llamada a `IsPlayServicesAvailable`):
...
El iniciador de la aplicación `Intent` se activa cuando el usuario pulsa su mensaje de notificación, por lo que este código registrará los datos que lo acompañan en el `Intent` en la ventana de salida. Si se debe desencadenar un `Intent` diferente, el campo de `click_action` del mensaje de notificación se debe establecer en ese `Intent` (el iniciador `Intent` se utiliza cuando no se especifica ningún `click_action`).
...
[![se muestra el botón del token de registro](remote-notifications-with-fcm-images/06-log-token-sml.png)](remote-notifications-with-fcm-images/06-log-token.png#lightbox)
...
La cadena larga etiquetada con **token** es el token de identificador de instancia que se pegará en la consola de Firebase &ndash; seleccionar y copiar esta cadena en el portapapeles. Si no ve un token de identificador de instancia, agregue la siguiente línea al principio del método `OnCreate` para comprobar que **Google-Services. JSON** se analizó correctamente:

```csharp
Log.Debug(TAG, "google app id: " + GetString(Resource.String.google_app_id));
```

El valor `google_app_id` registrado en la ventana de salida debe coincidir con el valor `mobilesdk_app_id` registrado en **Google-Services. JSON**.
...
[![las listas extras de intención de la clave, el identificador del mensaje y la clave de contraer](remote-notifications-with-fcm-images/13-intent-extras-sml.png)](remote-notifications-with-fcm-images/13-intent-extras.png#lightbox)
...
Para agregar un icono de notificación predeterminada personalizado, agregue el icono al directorio **Resources/drawable** , edite **archivo AndroidManifest. XML**e inserte el siguiente elemento de `<meta-data>` en la sección `<application>`:
...
Edite **Resources/layout/main. axml** y agregue la siguiente declaración de `Button` inmediatamente después del elemento `Button` anterior:
...
Este código busca el botón **suscribirse a notificación** en el diseño y asigna el controlador de clic al código que llama a `FirebaseMessaging.Instance.SubscribeToTopic`, pasando el tema suscrito, _News_. Cuando el usuario pulsa el botón **subscribe** , la aplicación se suscribe al tema _News_ . En la siguiente sección, se enviará un mensaje de tema de _noticias_ desde la GUI de notificaciones de la consola de Firebase.
...
[![ventana de salida muestra el mensaje sincronización correcta del tema](remote-notifications-with-fcm-images/15-topic-sync-sml.png)](remote-notifications-with-fcm-images/15-topic-sync.png#lightbox)
...
6. Compruebe la ventana de salida del IDE para ver **/topics/News** en la salida del registro:
    ...
    [![mensaje de/topic/News se muestra](remote-notifications-with-fcm-images/17-message-arrived-sml.png)](remote-notifications-with-fcm-images/17-message-arrived.png#lightbox)
...
[![el mensaje de tema aparece como una notificación](remote-notifications-with-fcm-images/18-other-news-sml.png)](remote-notifications-with-fcm-images/18-other-news.png#lightbox)
...
El servicio `FirebaseMessagingService` es responsable de recibir y procesar los mensajes de Firebase. Cada aplicación debe subclase de este tipo e invalidar el `OnMessageReceived` para procesar un mensaje entrante. Cuando una aplicación está en primer plano, la devolución de llamada de `OnMessageReceived` siempre controlará el mensaje.
...
Tenga en cuenta que el filtro de intención de `MESSAGING_EVENT` debe declararse de forma que los mensajes FCM nuevos se dirijan a `MyFirebaseMessagingService`:

```csharp
[IntentFilter(new[] { "com.google.firebase.MESSAGING_EVENT" })]
```

Cuando la aplicación cliente recibe un mensaje de FCM, `OnMessageReceived` extrae el contenido del mensaje del objeto `RemoteMessage` pasado llamando a su método `GetNotification`. Después, registra el contenido del mensaje para que se pueda ver en la ventana de salida del IDE:
...
> [!NOTE]
> Si establece puntos de interrupción en `FirebaseMessagingService`, la sesión de depuración puede alcanzar o no estos puntos de interrupción debido a la forma en que FCM entrega los mensajes.
...
En este ejemplo restante, el mensaje FCM entrante se convertirá en una notificación local que se inicia mientras la aplicación se ejecuta en primer plano. Edite **MyFirebaseMessageService.CS** y agregue las siguientes instrucciones de `using`:
...
Como resultado de estos cambios, se ejecutará `SendNotification` cada vez que se reciba una notificación mientras la aplicación está en primer plano y la notificación aparecerá en el área de notificación.
...
- Los mensajes de &ndash; de **notificación** se enviarán a la **bandeja del sistema**. Aparecerá una notificación local. Cuando el usuario pulsa la notificación, se iniciará la aplicación.
- Los mensajes de &ndash; de **datos** se controlarán mediante `OnMessageReceived`.
- **Tanto** los mensajes de &ndash; que tienen una notificación como una carga de datos se entregarán a la bandeja del sistema. Cuando se inicia la aplicación, la carga de datos aparecerá en el `Extras` del `Intent` que se usó para iniciar la aplicación.
...
Esta vez, el mensaje que se registró en la ventana de salida también se empaqueta en una nueva notificación &ndash; el icono de notificación aparece en la bandeja de notificación mientras la aplicación se ejecuta en primer plano:

[![icono de notificación del mensaje de primer plano](remote-notifications-with-fcm-images/22-foreground-icon-sml.png)](remote-notifications-with-fcm-images/22-foreground-icon.png#lightbox)
...
- [FCMNotifications (ejemplo)](https://docs.microsoft.com/samples/xamarin/monodroid-samples/firebase-fcmnotifications)
