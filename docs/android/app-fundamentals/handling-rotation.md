...
Esto incluye la compatibilidad para:

- *Recursos* &ndash; de diseño Especificar los archivos de diseño que se inflan para cada orientación.

- *Recursos* &ndash; Dibujables Que especifica qué drawables se cargan para cada orientación.
...
Este proyecto crea un único archivo **Main. axml** en la carpeta **recursos/diseño** . Cuando se llama al método de la actividad `OnCreate` , se infla la vista definida en **Main. axml,** que declara un botón como se muestra en el código XML siguiente:
...
Si el dispositivo se gira a la orientación horizontal, se llama de nuevo al método de la actividad `OnCreate` y se infla el mismo archivo **Main. axml** , tal y como se muestra en la siguiente captura de pantalla:
...
Además de la carpeta de diseño (que de forma predeterminada es vertical y también se puede llamar explícitamente al *Puerto de diseño* mediante la inclusión de una carpeta denominada `layout-land` ), una aplicación puede definir las vistas que necesita en el entorno sin ningún cambio de código.
...
Por ejemplo, suponga que el proyecto incluye una imagen denominada Monkey.png en la carpeta **Resources/drawable** , donde se hace referencia a la dibujable desde un `ImageView` en XML similar al siguiente:
...
[![Una versión diferente de Monkey.png se muestra en los modos vertical y horizontal](handling-rotation-images/03.png)](handling-rotation-images/03.png#lightbox)
...
Por ejemplo, considere una interfaz de usuario que consta de un único `TextView` control agregado a `RelativeLayout` , tal como se muestra en el código siguiente.
...
Además de controlar todo en `OnCreate` , una aplicación también puede impedir que una actividad se reinicie cuando cambie la orientación estableciendo `ConfigurationChanges` en el de la manera `ActivityAttribute` siguiente:
...
Aquí los `TextView's` parámetros de diseño se inicializan tanto horizontal como vertical. Las variables de clase contienen los parámetros, junto con el `TextView` propio, ya que la actividad no se volverá a crear cuando cambie la orientación. El código todavía utiliza `surfaceOrientartion` en `OnCreate` para establecer el diseño inicial de `TextView` . Después, `OnConfigurationChanged` controla todos los cambios de diseño posteriores.
...
Para ello, sigue el mismo procedimiento que usamos con un diseño mediante programación. Simplemente establezca `ConfigurationChanges` en `ActivityAttribute` , como hicimos en el `CodeLayoutActivity` anterior. Cualquier código que deba ejecutarse para el cambio de orientación se puede implementar de nuevo en el `OnConfigurationChanged` método.
...
- [Demostración de rotación (ejemplo)](/samples/xamarin/monodroid-samples/applicationfundamentals-rotationdemo)
